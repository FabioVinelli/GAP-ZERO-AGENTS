...
export async function generateScaffold(bp, outRoot) {
  const name = slug(bp.agentName);
  const dir = path.join(outRoot, name);
  const files = buildFiles(bp, name);
...
function buildFiles(bp, name) {
  ...
  const policyRules = bp.policyRules || [];
...
function wiringToolsPy(wiring) {
  ...
  return wiring
    .map((w, i) => {
      const fn = slug(w.step).replace(/-/g, "_") || `tool_${i}`;
    ...
    registry.register(ToolSpec(
        ...
        permission="write-with-validation" if ${JSON.stringify(w.reversibility)} == "irreversible" else "write",
        input_schema={"type": "object", "properties": {"payload": {"type": "string"}}, "required": []},
        execute=${fn}_execute,
        landing_check=${fn}_landing,
        risk_class=${JSON.stringify(w.reversibility === "irreversible" ? "irreversible" : "low")},
    ))`;
    })
    .join("\n\n");
}
